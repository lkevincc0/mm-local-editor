...
The app ships two switchable themes — `classic` (the default, matching legacy Bootstrap styling) and `modern` (warm, minimal editorial). `useTheme()` from `src/components/context/ThemeContext.tsx` exposes `{theme, mode, setTheme, setMode, toggleTheme, toggleMode}`; `mode` selects classic vs modern layout & interactions independently of the visual theme. Both are persisted to localStorage (`ammber/ui-theme` and `ammber/ui-mode`) and reflected as `data-theme` / `data-mode` on `<html>`.

Theme colors live as CSS custom properties in `src/styles/themes.css`, which has one `:root[data-theme='...']` block per theme. JS-side colors that CSS cannot reach (maxgraph canvas styles, inline styles, third-party components) live in `src/components/utils/themeTokens.ts`, keyed by theme name. Switching is a single button (`src/components/SettingsMenu.tsx`, used in both the project edit header and the Home page) that flips `theme` and `mode` together between classic and modern.

Rule: never hardcode a color — every new color must be a token defined in both theme blocks of `themes.css` (and in `themeTokens.ts` when JS-side code needs it). The modern palette is warm paper `#f7f6f0`, near-black `#11110f`, muted text `#6d6c66`, borders `#deddd6`, deep purple `#25213e`, and purple accent `#6b51c9`. Prefer generous whitespace, responsive `clamp()` sizing, compact copy, 7–14px corner radii, and soft restrained shadows. Avoid photographic page backgrounds, decorative clutter, and dense overlapping text.
